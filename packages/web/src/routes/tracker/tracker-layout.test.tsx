...
function mount(path = '/tracker', missing = false) {
  ...
  vi.stubGlobal('fetch', vi.fn(async (input: RequestInfo | URL) => {
    ...
    if (url.endsWith('/tracker/OPS-1')) return json({ available: true, item: item('OPS-1') })
    if (url.endsWith('/tracker/OPS-2')) return json(missing ? { available: false, code: 'unavailable', reason: 'Offline' } : { available: true, item: item('OPS-2') })
