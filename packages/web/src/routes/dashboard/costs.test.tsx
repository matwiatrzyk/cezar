...
it.each([false, true])('keeps revoked ranking policy over older cached sorts (restore=%s) and accepts fresh recovery', async restore => {
  ...
  expect(screen.queryAllByText('$0.000012')).toHaveLength(0)
  expect(container.querySelector('[data-dashboard-export]')?.getAttribute('data-dashboard-export')).not.toContain('costUsd')
  await act(() => client.cancelQueries({ queryKey: workspaceQueryKeys.dashboard }))
  vi.mocked(fetch).mockImplementation(fetcher)
  setData(fixture('ranking-recovered'))
  await act(() => client.invalidateQueries({ queryKey: workspaceQueryKeys.dashboard }))
  expect((await screen.findAllByText('$0.000012')).length).toBeGreaterThan(0)
})

it('keeps revoked detail policy when returning to cached sorts and restoring the Sheet', async () => {
  const { client, hideDetailCost, remount } = setup(undefined, fixture(), 'detail-cache-revoked')
  fireEvent.click(await screen.findByRole('button', { name: 'View tasks' }))
  const sheet = await screen.findByRole('dialog')
...
it('ignores an older in-flight detail policy and permits a later successful recovery', async () => {
  const { client, setData } = setup(undefined, { ...fixture(), sort: 'input' })
  await screen.findAllByText('$0.000012')
  let finishDetail!: (response: Response) => void
  vi.mocked(fetch).mockImplementationOnce(() => new Promise(resolve => { finishDetail = resolve }))
  fireEvent.click(screen.getByRole('button', { name: 'View tasks' }))
  const sheet = await screen.findByRole('dialog')
  await waitFor(() => expect(finishDetail).toBeTypeOf('function'))
  setData({ ...fixture(), visibility: { cost: false, tokens: true } })
  await act(() => client.invalidateQueries({ queryKey: [...workspaceQueryKeys.dashboard, 'costs', 'all'] }))
  await waitFor(() => expect(screen.queryAllByText('$0.000012')).toHaveLength(0))
