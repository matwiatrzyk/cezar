...
function contentChanged(a: DashboardCosts, b: DashboardCosts) {
  return (
    JSON.stringify([a.totals, a.projects, a.tasks, a.visibility]) !==
    JSON.stringify([b.totals, b.projects, b.tasks, b.visibility])
  )
}
...
function CostPeriod({
  ...
  const [accepted, setAccepted] = useState<DashboardCosts>()
  const data = accepted ?? query.data
  const latest = query.data ?? data
  const [panel, setPanel] = useState<string | null>(null)
  const policy = {
    cost: visibility.cost && latest?.visibility.cost !== false,
    tokens: visibility.tokens && latest?.visibility.tokens !== false,
  }
  ...
  return (
    <div className="space-y-4 p-4 text-sm">
      ...
      {latest && <Coverage coverage={latest.coverage} retry={() => void query.refetch()} />}
      {data && (
        <>
          ...
          {data.totals.tasks === 0 ? (
            <p>
              {latest?.coverage.projects.some((p) => p.state !== 'complete')
          ...
          <p className="text-muted-foreground">
            ...
            {latest?.invalidDateTasks
              ? ` ${latest.invalidDateTasks} tasks have missing or invalid creation dates${period === 'all' ? '; included in All time' : '; excluded from this period'}.`
              : ''}
          </p>
          {(policy.cost || policy.tokens) && (
            <>
              ...
              <CostProjectBars
                ...
                onSelect={setPanel}
              />
              <Button className="min-h-11" variant="outline" onClick={() => setPanel('')}>
                View tasks
              </Button>
            </>
          )}
      ...
      >
        <SheetContent className="w-full overflow-y-auto sm:max-w-xl [&>button]:min-h-11 [&>button]:min-w-11">
          <SheetHeader>
            <SheetTitle>{panel ? `${panel} tasks` : 'All tasks'}</SheetTitle>
            <SheetDescription>
...
function CostTasks({
  ...
  const [sort, setSort] = useState<Sort>(snapshot.sort)
...
function CostTaskPage({
  ...
  const [pagingSnapshot, setPagingSnapshot] = useState<DashboardCosts>()
  ...
  const [lastPolicy, setLastPolicy] = useState(visibility)
  ...
  const effectiveSort = choices(policy).includes(sort) ? sort : (choices(policy)[0] ?? 'cost')
  const [accepted, setAccepted] = useState<typeof query.data>()
  const [acceptedCount, setAcceptedCount] = useState(count)
  if (
    query.data &&
    (!accepted || (acceptedCount !== count && accepted.snapshotId === query.data.snapshotId))
  ) {
